...
* `!defender set <property> <value>` — Modifies a configuration parameter dynamically on the fly (e.g. `!defender set missilespeed 25`).
...
| `AllowMissileDamage` | `bool` | `true` | Allows player-made missiles to deal impact deformation damage. |
| `LargeGridMissileMinBlocks` | `int` | `5` | Minimum block count for a large grid missile. |
| `LargeGridMissileMaxBlocks` | `int` | `50` | Maximum block count for a large grid missile. |
| `SmallGridMissileMinBlocks` | `int` | `10` | Minimum block count for a small grid missile. |
...
| `MaxDeformationVelocity` | `float` | `110.0` | Extreme-speed collisions above this speed skip deformation to prevent server freezes. |
